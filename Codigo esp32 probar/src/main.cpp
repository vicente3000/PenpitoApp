...
void servo_cont_set(int ang) {
  int vel = constrain(ang, -100, 100);
  int pulse = SRV_CONT_STOP + SRV_CONT_TRIM + map(vel, -100, 100, -90, 90);
  pulse = constrain(pulse, 0, 180);
  srv_cont.write(pulse);
  Serial.print(F("Servo continuo vel=")); Serial.print(vel);
  Serial.print(F(" pwm=")); Serial.println(pulse);
}
