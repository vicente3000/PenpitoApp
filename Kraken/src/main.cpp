...
const char* MQTT_HOST = "192.168.1.100"; // IP de la PC donde corre Mosquitto
...
// ═══════════════════════════════════════════
const bool SIMULAR_MOTOR = true; // CAMBIAR A 'false' cuando conectes el NEMA17 físico

// CONSTANTES DE CALIBRACIÓN DE LA CUCHARA (Ajustar con el panel de calibración de la app)
const unsigned long TIEMPO_BAJAR_CUCHARA = 3000; // ms que demora en bajar al fondo
const unsigned long TIEMPO_AGITACION = 5000;     // ms que se queda revolviendo
const unsigned long TIEMPO_SUBIR_CUCHARA = 3100; // ms que demora en subir al tope
...
unsigned long pump_stop_time[7] = {0}; // Tiempos de apagado no bloqueante para bombas
...
int ice_cycle_state = 0; // 0 = abrir g1, 1 = cerrar g1, 2 = abrir g2, 3 = cerrar g2
int stir_state = 0;      // Estado de agitación: 0=bajar, 1=revolver, 2=subir, 3=detener
  ...
  for (int i = 0; i < NUM_BOMBAS; i++) {
    if (pump_stop_time[i] > 0) {
      if (now >= pump_stop_time[i]) {
        ...
        pump_stop_time[i] = 0;
        Serial.print("Bomba "); Serial.print(i + 1); Serial.println(" OFF (Tiempo completado)");
      }
    }
  }
  ...
  switch (activeStep) {
    case STEP_CUP_DISPENSER:
      ...
      if (now >= step_timer) {
        ...
        servo_pos(0, 180);
        delay(1200); // Retardo físico de caída de vaso (pequeño bloqueo tolerado en transicion)
        servo_pos(0, 90);
        delay(500);
    ...
    case STEP_ALCOHOL_DISPENSER:
      if (now >= step_timer && !waiting_for_pumps) {
        ...
        if (currentRecipeId == "piscola") {
          ...
          Serial.print("Piscola -> Pisco: "); Serial.print(ml_pisco); Serial.print("ml ("); Serial.print(t_pisco); Serial.println("ms)");
          Serial.print("Piscola -> Cola: "); Serial.print(ml_cola); Serial.print("ml ("); Serial.print(t_cola); Serial.println("ms)");
          
          digitalWrite(B_PIN[0], HIGH); pump_stop_time[0] = millis() + t_pisco;
          digitalWrite(B_PIN[6], HIGH); pump_stop_time[6] = millis() + t_cola;
        } 
          ...
          digitalWrite(B_PIN[2], HIGH); pump_stop_time[2] = millis() + t_gin;
          digitalWrite(B_PIN[3], HIGH); pump_stop_time[3] = millis() + t_campari;
          digitalWrite(B_PIN[4], HIGH); pump_stop_time[4] = millis() + t_vermut;
        } 
          ...
          digitalWrite(B_PIN[5], HIGH); pump_stop_time[5] = millis() + t_whisky;
          digitalWrite(B_PIN[3], HIGH); pump_stop_time[3] = millis() + t_campari;
          digitalWrite(B_PIN[4], HIGH); pump_stop_time[4] = millis() + t_vermut;
        }
          ...
          digitalWrite(B_PIN[5], HIGH); pump_stop_time[5] = millis() + t_whisky;
          digitalWrite(B_PIN[1], HIGH); pump_stop_time[1] = millis() + t_amaretto;
        }
        else if (currentRecipeId == "americano") {
          float ml_campari = (customAlcoholOz > 0) ? customAlcoholOz * 30.0 : 45.0;
          ...
          float ml_campari = (customAlcoholOz > 0) ? customAlcoholOz * 30.0 : 60.0;
          unsigned long t_campari = (ml_campari / b_ml_ps[3]) * 1000;
...
void mqttCallback(char* topic, byte* payload, unsigned int length) {
  ...
  if (cmd == "POWER") {
    ...
    if (type == "pump") {
      ...
      if (idx >= 0 && idx < NUM_BOMBAS) {
        ...
        Serial.print("Test Bomba "); Serial.print(pin); Serial.print(" por "); Serial.print(duration); Serial.println("ms");
      }
    } 
    else if (type == "servo") {
      int pin = doc["pin"] | 1;
      ...
      if (idx >= 0 && idx < 3) {
        servo_pos(idx, val);
        Serial.print("Test Servo "); Serial.print(pin); Serial.print(" a "); Serial.println(val);
      }
    } 
      ...
      Serial.print("Test Servo Continuo velocidad="); Serial.print(val);
      Serial.print(" por "); Serial.print(duration); Serial.println("ms");
    } 
    else if (type == "motor") {
      if (!SIMULAR_MOTOR) {
        motor_steps(val);
      } else {
        Serial.print("[SIMULACION] Moviendo motor steps: "); Serial.println(val);
      }
    }
  ...
  stateObj["errorMessage"] = errorMessage.length() ? errorMessage.c_str() : nullptr;
  ...
  stateObj["activeStepId"] = activeStep == STEP_NONE ? nullptr : stepToString(activeStep);
...
void publishState() {
  ...
  stateDoc["errorMessage"] = errorMessage.length() ? errorMessage.c_str() : nullptr;
  stateDoc["currentRecipeId"] = currentRecipeId.length() ? currentRecipeId.c_str() : nullptr;
  stateDoc["requestedIceCount"] = requestedIceCount;
  stateDoc["activeStepId"] = activeStep == STEP_NONE ? nullptr : stepToString(activeStep);
